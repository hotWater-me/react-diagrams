	...
	constructor(options: CustomDeleteItemsActionOptions = {}) {
		...
		super({
			...
			fire: (event: ActionEvent<React.KeyboardEvent>) => {
				...
				console.log('🚀 ~ selectedEntities', event);
				switch (code) {
					case 'Backspace':
						console.log(event.event, '触发了删除操作');
						break;
					case 'KeyC':
						console.log(event.event, '触发了复制操作');
						break;
					case 'KeyV':
						console.log(event.event, '触发了粘贴操作');
						break;
